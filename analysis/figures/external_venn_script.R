...
generate_venns <- function(cohort){
  
  active_analyses <- readr::read_rds("lib/active_analyses.rds")
  group <- active_analyses %>% filter(venn == TRUE) %>% distinct(outcome_group) %>% pull(outcome_group) 
  
  df <- read.csv(paste0(venn_res,"venn_diagram_number_check_",cohort, "_",group,".csv"))
  ...
  for(i in outcomes) {
    ...
    svglite::svglite(file = paste0(output_dir,"venn_diagram_",gsub("out_date_","",i),".svg"))
    grid.newpage()
