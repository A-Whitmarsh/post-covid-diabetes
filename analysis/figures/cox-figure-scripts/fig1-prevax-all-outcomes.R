...
prevax_figures_1 <- function(cohort) {
  ...
  #---------------------------------------------#
  hr_files=list.files(path = prevax_results_dir, pattern = paste0("_",cohort))
  hr_files=hr_files[endsWith(hr_files,".csv")]
  hr_files=paste0(prevax_results_dir,"/", hr_files)
  ...
  levels(main_estimates$model) <- list("Age/sex/region"="mdl_agesex", "Extensive"="mdl_max_adj")
  ...
  for(c in cohort){
    for (o in unique(main_estimates_reduced$event)) {
      ...
      ggplot2::ggplot(data=df,
        ...
        ggtitle(paste0(str_to_title(df$outcome))) +
  ...
  for(c in cohort){
    ...
    ggplot2::ggsave(paste0(output_dir,"diabetes_",c,"_figure_main_all.png"), height = 297, width = 210, unit = "mm", dpi = 600, scale = 1)
  }
