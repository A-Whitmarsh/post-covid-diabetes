...
dir <- "/Users/kt17109/OneDrive - University of Bristol/ehr_postdoc/projects/post-covid-diabetes"
setwd(dir)

results_dir <- "/Users/kt17109/OneDrive - University of Bristol/Documents - grp-EHR/Projects/post-covid-diabetes/prevax/reports/pre-vax-report-22-07-22/"
output_dir <- "/Users/kt17109/OneDrive - University of Bristol/Documents - grp-EHR/Projects/post-covid-diabetes/prevax/reports/pre-vax-report-22-07-22/"
...
#---------------------------------------------#
hr_files=list.files(path = results_dir, pattern = c("suppressed_compiled_HR_results_t2dm_to_release","suppressed_compiled_HR_results_t2dm_to_release__non_hosp_force*"))
...
non_hosp <- as.data.frame(hr_file_paths[1])
main <- as.data.frame(hr_file_paths[2])
non_hosp <- subset(non_hosp, subgroup == "covid_pheno_non_hospitalised")
main <- subset(main, subgroup == "main")
estimates <- rbind(main, non_hosp)

# replace redacted

# estimates[] <- lapply(estimates, gsub, pattern = "[Redacted]", replacement = "0", fixed = TRUE)

# Get estimates for main analyses and list of outcomes from active analyses
main_estimates <- subset(estimates, event %in% outcome_to_plot & term %in% term[grepl("^days",term)])
...
c="prevax"
group="diabetes"
subgroups <- c("main", "covid_pheno_non_hospitalised")

# Figure comparing main t2dm against non-hosp t2dm with full time points 

main_estimates$subgroup <- factor(main_estimates$subgroup, levels = c("main", "covid_pheno_non_hospitalised"))
...
for(c in cohort){
  ...
  ggplot2::ggsave(paste0(output_dir,"diabetes_",c,"_figure_main_compare_non_hosp.png"), height = 150, width = 250, unit = "mm", dpi = 600, scale = 1)
}
